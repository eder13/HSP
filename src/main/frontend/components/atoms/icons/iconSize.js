const ICONSIZE = {
    SIZE_0_25X: { transform: 'scale(0.25)' },
    SIZE_0_5X: { transform: 'scale(0.5)' },
    SIZE_0_75X: { transform: 'scale(0.75)' },
    SIZE_1X: { transform: 'scale(1)' },
    SIZE_1_25X: { transform: 'scale(1.25)' },
    SIZE_1_5X: { transform: 'scale(1.5)' },
    SIZE_1_75X: { transform: 'scale(1.75)' },
    SIZE_2X: { transform: 'scale(2)' },
    SIZE_2_25X: { transform: 'scale(2.25)' },
    SIZE_2_5X: { transform: 'scale(2.5)' },
    SIZE_2_75X: { transform: 'scale(2.75)' },
    SIZE_3X: { transform: 'scale(3)' }
};

export default ICONSIZE;
