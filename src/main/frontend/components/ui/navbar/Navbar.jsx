...
const getLiContent = (isLoggedIn, isMobileNavbarActive) => {
    ...
    const linkClasses = cssClassNamesHelper([
        'nav-link',
        'mx-3',
        isMobileNavbarActive && 'text-center'
    ]);

    const li1 = (<Link className={`${linkClasses} active`} to="/">
                     {isLoggedIn ? 'Meine Uploads & Downloads' : 'Home' }
                     <span className="visually-hidden">(current)</span>
                </Link>);
    const li2 = isLoggedIn ? <Link className={linkClasses} to="">Unterrichtsmaterialien</Link> : <a className={`${linkClasses}`} href="#">Über x</a>;
    const li3 = !isLoggedIn && <a className={`${linkClasses}`} href="#">FAQ</a>;
